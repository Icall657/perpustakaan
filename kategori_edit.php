...
if ($id) {
    $result = $conn->query("SELECT * FROM kategori_buku WHERE id = $id");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt->execute()) {
        ...
        $pesan = "<div class='bg-green-100 text-green-700 p-3 rounded mb-4'>Kategori berhasil diperbarui! Mengalihkan...</div>";
    } else {
        $pesan = "<div class='bg-red-100 text-red-700 p-3 rounded mb-4'>Gagal memperbarui kategori.</div>";
    }
}
?>
<?php
...
<body class="bg-gray-100">
    ...
    <div class="container mx-auto mt-8 p-6 bg-white shadow-md rounded-lg w-96">
        <h2 class="text-2xl font-semibold mb-4 text-gray-700">Edit Kategori</h2>
        ...
        <form method="POST">
            <div class="mb-4">
                ...
                <input type="text" name="nama_kategori" value="<?= $data['nama_kategori'] ?? '' ?>" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring focus:border-blue-300" required>
            </div>
